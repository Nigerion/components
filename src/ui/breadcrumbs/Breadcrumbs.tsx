...
export const Breadcrumbs: React.FC<BreadcrumbsProps> = ({
    separator = <ChevronRight className={styles.separator} strokeWidth={1.5} size={20} />,
    maxItems = 3,
    className,
    itemAfterCount = 1,
    itemBeforeCount = 1,
    ...
    const renderBreadcrumbItem = (
        ...
        <li key={crumb.path} className={styles.breadcrumbItem}>
            ...
            {crumb.disabled || isLast ? (
                ...
                <button type="button" className={styles.link} onClick={() => goTo(crumb.path)}>
                    {crumb.label}
                </button>
            )}
        </li>
    ...
    const renderCollapsedBreadcrumbs = () => {
        ...
        return (
            <>
                {firstItems.map((crumb, index) => renderBreadcrumbItem(crumb, index, false))}
            ...
            <ol className={styles.breadcrumbsList}>
                {shouldCollapse
                    ...
                    : breadcrumbs.map((crumb, index) =>
                          renderBreadcrumbItem(crumb, index, index === breadcrumbs.length - 1),
                      )}
            </ol>
        </nav>
    );
